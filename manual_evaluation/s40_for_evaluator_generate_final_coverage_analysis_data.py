...
def export_results(evaluated_model_label=O_GEMMA_LABEL,
    ...
    if evaluator_model_1 is not None:
        ...
        with open(data_file_1, encoding="utf-8") as json_file:
            ...
            for item in items:
                item_evaluations = {}
                if item.get(evaluator_model_1 + "_original_reasoning_text_breakdown"):
                    item_evaluations[evaluator_model_1 + "_original_reasoning_text_breakdown"] = item.get(evaluator_model_1 + "_original_reasoning_text_breakdown")
                if item.get(evaluator_model_1 + "_justification_breakdown"):
                    item_evaluations[evaluator_model_1 + "_justification_breakdown"] = item.get(evaluator_model_1 + "_justification_breakdown")
    ...
    if evaluator_model_2 is not None:
        ...
        with open(data_file_2, encoding="utf-8") as json_file:
            ...
            for item in items:
                item_evaluations = {}
                if item.get(evaluator_model_2 + "_original_reasoning_text_breakdown"):
                    item_evaluations[evaluator_model_2 + "_original_reasoning_text_breakdown"] = item.get(evaluator_model_2 + "_original_reasoning_text_breakdown")
                if item.get(evaluator_model_2 + "_justification_breakdown"):
                    item_evaluations[evaluator_model_2 + "_justification_breakdown"] = item.get(evaluator_model_2 + "_justification_breakdown")
    ...
    # for item in items[max_items:]:
        ...
        if results_items_dict_1.get(item["item_id"]):
            if results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + "_original_reasoning_text_breakdown"):
                item_dict[evaluator_model_1 + "_reference_arguments"] = results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + "_original_reasoning_text_breakdown")
            if results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + "_justification_breakdown"):
                item_dict[evaluator_model_1 + "_proposed_arguments"] = results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + "_justification_breakdown")
        ...
        if results_items_dict_2.get(item["item_id"]):
            if results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + "_original_reasoning_text_breakdown"):
                item_dict[evaluator_model_2 + "_reference_arguments"] = results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + "_original_reasoning_text_breakdown")
            if results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + "_justification_breakdown"):
                item_dict[evaluator_model_2 + "_proposed_arguments"] = results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + "_justification_breakdown")
