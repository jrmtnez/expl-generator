...
def export_results(evaluated_model_label=O_GEMMA_LABEL,
    ...
    if evaluator_model_1 is not None:
        ...
        with open(data_file_1, encoding="utf-8") as json_file:
            ...
            for item in items:
                item_evaluations = {}
                if item.get(evaluator_model_1 + "_original_reasoning_text_main_argument"):
                    item_evaluations[evaluator_model_1 + "_original_reasoning_text_main_argument"] = item.get(evaluator_model_1 + "_original_reasoning_text_main_argument")
                if item.get(evaluator_model_1 + "_justification_main_argument"):
                    item_evaluations[evaluator_model_1 + "_justification_main_argument"] = item.get(evaluator_model_1 + "_justification_main_argument")
    ...
    if evaluator_model_2 is not None:
        ...
        with open(data_file_2, encoding="utf-8") as json_file:
            ...
            for item in items:
                item_evaluations = {}
                if item.get(evaluator_model_2 + "_original_reasoning_text_main_argument"):
                    item_evaluations[evaluator_model_2 + "_original_reasoning_text_main_argument"] = item.get(evaluator_model_2 + "_original_reasoning_text_main_argument")
                if item.get(evaluator_model_2 + "_justification_main_argument"):
                    item_evaluations[evaluator_model_2 + "_justification_main_argument"] = item.get(evaluator_model_2 + "_justification_main_argument")
    ...
    # for item in items[max_items:]:
        ...
        if results_items_dict_1.get(item["item_id"]):
            if results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + "_original_reasoning_text_main_argument"):
                item_dict[evaluator_model_1 + "_main_reference_argument"] = results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + "_original_reasoning_text_main_argument")
            if results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + "_justification_main_argument"):
                item_dict[evaluator_model_1 + "_main_proposed_argument"] = results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + "_justification_main_argument")
        ...
        if results_items_dict_2.get(item["item_id"]):
            if results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + "_original_reasoning_text_main_argument"):
                item_dict[evaluator_model_2 + "_main_reference_argument"] = results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + "_original_reasoning_text_main_argument")
            if results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + "_justification_main_argument"):
                item_dict[evaluator_model_2 + "_main_proposed_argument"] = results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + "_justification_main_argument")
        ...
        # if item_dict[evaluator_model_1 + measure_name] == "OK" and item_dict[evaluator_model_2 + measure_name] == "OK":
        #     results.append(item_dict)
...
if __name__ == '__main__':
    ...
    measure_name = "_item_coverage_main_argument_auto"
    measure_reasoning = "_item_coverage_main_argument_analysis_auto"
