...
def export_results(evaluated_model_label=O_GEMMA_LABEL,
                   ...
                   measure_name=None, max_items=200):
    ...
    for item in items[:max_items]:
        ...
        item_dict["reference_explanation"] = item["original_reasoning_text"]
        item_dict["generated_explanation"] = item["justification"]

        item_dict["generating model"] = evaluated_model_label

        evaluator_model_1_measure = ""
        evaluator_model_2_measure = ""
        if results_items_dict_1.get(item["item_id"]):
            if results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + measure_name):
                evaluator_model_1_measure = results_items_dict_1.get(item["item_id"]).get(evaluator_model_1 + measure_name)

        if results_items_dict_2.get(item["item_id"]):
            if results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + measure_name):
                evaluator_model_2_measure = results_items_dict_2.get(item["item_id"]).get(evaluator_model_2 + measure_name)

        if evaluator_model_1_measure == "NOK" or evaluator_model_2_measure == "NOK":
...
if __name__ == '__main__':
    ...
    with open(join(RESULTS_ANALYSIS_PATH, FINAL_EXPLANATION_FILE_NAME % "combined"), "w", encoding="utf-8") as write_file:
